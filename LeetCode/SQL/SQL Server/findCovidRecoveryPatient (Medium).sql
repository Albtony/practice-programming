/*
Source: https://leetcode.com/problems/find-covid-recovery-patients/description/
Diff: Medium
Dialect: SQL Server
...
WITH first_positive AS (
    SELECT 
        patient_id,
        MIN(CASE WHEN result = 'Positive' THEN test_date END) AS test_date
    FROM covid_tests
    GROUP BY patient_id
), 
first_negative AS (
    SELECT 
        ct.patient_id,
        MIN(CASE WHEN result = 'Negative' AND ct.test_date >= fp.test_date THEN ct.test_date END) AS test_date
    FROM covid_tests ct
...
SELECT 
    ...
    p.patient_name,
    p.age,
    DATEDIFF(day, fp.test_date, fn.test_date) AS recovery_time
FROM patients p
JOIN first_positive fp ON fp.patient_id = p.patient_id
JOIN first_negative fn ON fn.patient_id = p.patient_id
WHERE fp.test_date IS NOT NULL AND fn.test_date IS NOT NULL
