...
improvement_scores AS (
    SELECT DISTINCT
        n1.employee_id, 
        n1.rating - n3.rating AS improvement_score
    FROM numbered n1
    JOIN numbered n2
    ON n1.employee_id = n2.employee_id
    JOIN numbered n3
    ON n1.employee_id = n3.employee_id
    WHERE 
        (n1.rn = 1 AND n3.rn = 3) AND
        (n1.rating > n2.rating AND n2.rating > n3.rating)
)
...
SELECT 
    ...
    i.improvement_score
FROM employees e
JOIN improvement_scores i
ON 
    e.employee_id = i.employee_id AND 
    i.improvement_score > 0
ORDER BY i.improvement_score DESC, e.name
