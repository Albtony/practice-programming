...
Return the result table of reportable category pairs ordered by customer_count in descending order, and in case of a tie, by category1 in 
ascending order lexicographically, and then by category2 in ascending order.
...
Other category pairs like Clothing-Sports (only 2 customers: Users 1 and 4) and Books-Kitchen (only 1 customer: User 3) have fewer than 3 
shared customers and are not included in the result.
The result is ordered by customer_count in descending order. Since all pairs have the same customer_count of 3, they are ordered by category1 
...
JOIN productInfo pi2 ON pi1.category < pi2.category
JOIN productPurchases pp1 ON pi1.product_id = pp1.product_id
JOIN productPurchases pp2 ON pi2.product_id = pp2.product_id AND pp1.user_id = pp2.user_id
GROUP BY pi1.category, pi2.category
HAVING COUNT(DISTINCT pp1.user_id) >= 3
ORDER BY customer_count DESC, pi1.category, pi2.category