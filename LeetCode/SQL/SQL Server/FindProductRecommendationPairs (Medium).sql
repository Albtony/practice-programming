/*
Source: https://leetcode.com/problems/find-product-recommendation-pairs/description/
Diff: Medium
Dialect: SQL Server
...
Return the result table ordered by customer_count in descending order, and in case of a tie, by product1_id in ascending order, and 
...
WITH PairPurchases AS (
    SELECT 
        pdp1.product_id AS product1_id,
        pdp2.product_id AS product2_id
    FROM ProductPurchases pdp1
    JOIN ProductPurchases pdp2
    ON 
        pdp1.user_id = pdp2.user_id AND
        pdp1.product_id < pdp2.product_id
),
TotalPairPurchases AS (
    SELECT 
        prp.*,
        COUNT(*) AS customer_count
    FROM PairPurchases prp
    GROUP BY prp.product1_id, prp.product2_id
)
SELECT 
    tpp.product1_id,
    tpp.product2_id,
    pdi1.category AS product1_category,
    pdi2.category AS product2_category,
    tpp.customer_count
FROM TotalPairPurchases tpp
JOIN ProductInfo pdi1 ON tpp.product1_id = pdi1.product_id 
JOIN ProductInfo pdi2 ON tpp.product2_id = pdi2.product_id 
WHERE tpp.customer_count >= 3
ORDER BY tpp.customer_count DESC, tpp.product1_id, tpp.product2_id