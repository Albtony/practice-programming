/*
Source: https://leetcode.com/problems/analyze-subscription-conversion/
Diff: Medium
Dialect: SQL Server
...
A subscription service wants to analyze user behavior patterns. The company offers a 7-day free trial, after which users can subscribe to a 
...
WITH calc AS (
    SELECT
        user_id,
        ROUND(
            AVG(CASE WHEN activity_type = 'free_trial' THEN activity_duration*1.0 END),
        2) AS trial_avg_duration,
        ROUND(
            AVG(CASE WHEN activity_type = 'paid' THEN activity_duration*1.0 END),
