...
Salary Budget: For each manager, calculate the total salary budget they control (sum of salaries of all employees under them, including indirect reports, 
plus their own salary).
...
descendant_entry AS (
    ...
    SELECT 
        ...
        e.salary
    FROM employees e
    JOIN descendant_entry de ON e.manager_id = de.employee_id
), 
descendant_info AS (
    SELECT 
        root_id,
    ...
    FROM descendant_entry de
    GROUP BY root_id
)

SELECT DISTINCT
    di.root_id AS employee_id,
    e.employee_name,
    h.level,
    di.team_size,
    di.budget
FROM descendant_info di
JOIN hierarchy h ON di.root_id = h.employee_id
JOIN employees e ON h.employee_id = e.employee_id
ORDER BY h.level, di.budget DESC, e.employee_name