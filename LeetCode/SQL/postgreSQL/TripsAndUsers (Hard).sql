...
The cancellation rate is computed by dividing the number of canceled (by client or driver) requests with unbanned users by the 
...
WITH stats AS (
    SELECT 
        COUNT(*) FILTER(WHERE t.status != 'completed') AS Cancelled_orders,
...
SELECT
    Day,
    ROUND(Cancelled_orders::Decimal / Total_orders, 2) AS "Cancellation Rate"
FROM stats
WHERE 
    Day >= '2013-10-01' AND
    DAY <= '2013-10-03' AND
