...
A senior analyst is interested to know the activation rate of specified users in the emails table. Write a query 
to find the activation rate. Round the percentage to 2 decimal places.
...
SELECT 
    ROUND(
        1.0 *
        COUNT(DISTINCT e.email_id) FILTER(WHERE t.signup_action = 'Confirmed') / 
