...
a1, a2, ..., an. But as Mom was running out of time, she didn't split the coins for you two. So she scribbled a note 
...
sum of values is strictly more than the sum of values of the remaining coins. On this basis, determine what minimum number 
of coins you need to take to divide them in the described manner.
*/
...
int main(){
    int ncoins, coins[100];
    int mySum = 0, currSum = 0;
    ...
    sort(coins, coins + ncoins, greater<int>());    
    for(int i = 0; i < ncoins; i++){
        mySum += coins[i];
        currSum -= coins[i];
