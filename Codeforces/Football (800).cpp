...
on a piece of paper. To simplify the situation he depicted it as a string consisting of zeroes and ones. A zero corresponds 
to players of one team; a one corresponds to players of another team. If there are at least 7 players of some team standing 
one after another, then the situation is considered dangerous. For example, the situation 00100110111111101 is dangerous and 
...
int main(){
    string playerPos;
    int dangerLevel, treshold;

    dangerLevel = 1; treshold = 7;
    cin >> playerPos;

    for(int i = 0; i < playerPos.length(); i++)
        if(playerPos[i] == playerPos[i+1]){
            ++dangerLevel;
            if(dangerLevel == treshold){
                cout << "YES";
                return 0;
            }
        } else
